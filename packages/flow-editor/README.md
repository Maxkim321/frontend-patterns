...
同一个 `FlowEditor` 组件：
- 审批流配置 = 开始/AI 预审/条件分支/人工复核/结束 + 「开始禁入线、结束禁出线、分支最多 2 出线」
- 数据编排配置 = 数据源/算子/输出 + 「算子最多 2 入线」
- 切换只需换 props（materials + validators + initialGraph），编辑器一行不改。
  demo 为两套业务都预置了初始图（审批流预置完整 AI 审批链），切换即整体重挂载。
